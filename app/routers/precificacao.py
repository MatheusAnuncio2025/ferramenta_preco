...
router = APIRouter(
    prefix="/api/precificacao",
    tags=["Precificação e Dados da Calculadora"]
)

@router.get("/categorias-precificacao", response_model=List[models.CategoriaPrecificacao])
async def get_categorias_para_calculadora(user: dict = Depends(dependencies.get_current_user)):
...
async def get_dados_para_calculo(sku: str, loja_id: str, user: dict = Depends(dependencies.get_current_user)):
    ...
    if not config_loja_data:
        raise HTTPException(status_code=404, detail=f"Configuração da loja '{loja_id}' não encontrada.")
...
async def salvar_precificacao(data: models.PrecificacaoCore, user: dict = Depends(dependencies.get_current_user)):
    ...
    try:
        ...
        row_data.update({"id": record_id, "data_calculo": services.datetime.utcnow(), "calculado_por": user_email})
...
@router.get("", response_model=models.PrecificacaoListResponse)
async def listar_precificacoes(
    marketplace: Optional[str]=None, id_loja: Optional[str]=None, sku: Optional[str]=None,
    titulo: Optional[str]=None, plano: Optional[str]=None, categoria: Optional[str]=None,
    page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100),
    user: dict = Depends(dependencies.get_current_user)
):
    filters = {"marketplace": marketplace, "id_loja": id_loja, "sku": sku, "titulo": titulo, "plano": plano, "categoria": categoria}
...
@router.get("/historico/{sku}", response_model=List[dict])
...
@router.get("/{id}/edit-data", response_model=models.EditPageData)
async def get_data_for_edit_page(id: str, user: dict = Depends(dependencies.get_current_user)):
    try:
        ...
        loja_id = services.get_loja_id_by_marketplace_and_loja(precificacao_base['marketplace'], precificacao_base['id_loja'])
        if not loja_id:
            raise HTTPException(status_code=404, detail="Configuração da loja não encontrada.")
        config_loja = await services.get_loja_details(loja_id)
        produto_atual_data = services.fetch_product_data(precificacao_base['sku']) or {"sku": precificacao_base['sku']}
...
@router.post("/campanha", status_code=201)
