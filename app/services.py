...
def bulk_update_prices(payload: models.BulkUpdatePayload, user_email: str):
    ...
    if payload.action == models.UpdateAction.set_custo_unitario:
        field_to_update = "custo_unitario"; param_type = "FLOAT64"
    elif payload.action == models.UpdateAction.set_categoria:
        field_to_update = "categoria_precificacao"; param_type = "STRING"
    else:
        raise ValueError(f"Ação de atualização em massa '{payload.action.value}' não é suportada.")
    query = f""" UPDATE `{TABLE_PRECIFICACOES_SALVAS}` SET {field_to_update} = @value, calculado_por = @user_email, data_calculo = CURRENT_TIMESTAMP() WHERE id IN UNNEST(@ids) """
    params = [bigquery.ScalarQueryParameter("value", param_type, payload.value), bigquery.ScalarQueryParameter("user_email", "STRING", user_email), bigquery.ArrayQueryParameter("ids", "STRING", payload.ids)]
    job_config = bigquery.QueryJobConfig(query_parameters=params)
...
def get_linked_campaigns(base_id: str) -> List[Dict[str, Any]]:
    query = f"SELECT pc.*, c.nome as nome_campanha FROM `{TABLE_PRECIFICACOES_CAMPANHA}` pc JOIN `{TABLE_CAMPANHAS_ML}` c ON pc.campanha_id = c.id WHERE pc.precificacao_base_id = @base_id"
...
def get_campaign_pricing_details(item_id: str) -> Optional[Dict[str, Any]]:
    query = f"SELECT pc.*, c.nome as nome_campanha, pb.sku, pb.titulo FROM `{TABLE_PRECIFICACOES_CAMPANHA}` pc JOIN `{TABLE_CAMPANHAS_ML}` c ON pc.campanha_id = c.id JOIN `{TABLE_PRECIFICACOES_SALVAS}` pb ON pc.precificacao_base_id = pb.id WHERE pc.id = @id"
...
def get_price_history_for_sku(sku: str) -> List[Dict[str, Any]]:
    query = f"SELECT * FROM `{TABLE_LOGS}` WHERE action = 'UPDATE_PRICING' AND JSON_EXTRACT_SCALAR(details, '$.sku') = @sku ORDER BY timestamp DESC"
    params = [bigquery.ScalarQueryParameter("sku", "STRING", sku)]
    results = [dict(row) for row in execute_query(query, params)]
    for item in results:
        for key, value in item.items():
...
def get_all_users() -> List[Dict[str, Any]]:
    ...
    for user in results:
        for key, value in user.items():
            if isinstance(value, (datetime, date)): user[key] = value.isoformat()
    return results
...
def save_all_campaigns(campaigns_list: List[Dict[str, Any]]):
    ...
    if ids_na_ui:
        delete_query = f"DELETE FROM `{TABLE_CAMPANHAS_ML}` WHERE id NOT IN UNNEST(@ids)"
        params = [bigquery.ArrayQueryParameter("ids", "STRING", ids_na_ui)]
    ...
    for campaign in campaigns_list:
        ...
        merge_query = f"""
            ...
            WHEN MATCHED THEN 
                UPDATE SET {", ".join([f"T.{k} = S.{k}" for k in campaign.keys() if k != 'id'])}
            WHEN NOT MATCHED THEN 
                INSERT ({cols}) VALUES ({cols})
        """
        params = [bigquery.ScalarQueryParameter(k, "STRING" if v is None or isinstance(v, str) else "DATE" if isinstance(v, date) else "NUMERIC", v) for k, v in campaign.items()]
...
async def get_loja_details(loja_id: str) -> Dict[str, Any]:
    ...
    results = [dict(row) for row in execute_query(query, params)]
    if not results or not results[0].get('configuracoes'): return {}
...
def save_loja_details(loja_id: str, detalhes_json: str):
    query = f"MERGE `{TABLE_LOJA_CONFIG_DETALHES}` T USING (SELECT @loja_id as loja_id, @config_json as configuracoes) S ON T.loja_id = S.loja_id WHEN MATCHED THEN UPDATE SET T.configuracoes = S.configuracoes WHEN NOT MATCHED THEN INSERT (loja_id, configuracoes) VALUES (S.loja_id, S.configuracoes)"
...
def get_dashboard_alert_data() -> Dict[str, List]:
    query_campanhas = f"SELECT * FROM `{TABLE_CAMPANHAS_ML}` WHERE data_fim BETWEEN CURRENT_DATE() AND DATE_ADD(CURRENT_DATE(), INTERVAL 7 DAY) ORDER BY data_fim ASC"
...
def get_profitability_by_category() -> List[Dict[str, Any]]:
    query = f"SELECT categoria_precificacao as label, SUM(lucro_classico + lucro_premium) as value FROM `{TABLE_PRECIFICACOES_SALVAS}` WHERE categoria_precificacao IS NOT NULL GROUP BY categoria_precificacao ORDER BY value DESC"
    return [dict(row) for row in execute_query(query)]

def get_profit_evolution() -> List[Dict[str, Any]]:
    query = f"SELECT FORMAT_TIMESTAMP('%Y-%m', data_calculo) as label, SUM(lucro_classico + lucro_premium) as value FROM `{TABLE_PRECIFICACOES_SALVAS}` WHERE data_calculo >= TIMESTAMP(DATE_SUB(CURRENT_DATE(), INTERVAL 6 MONTH)) GROUP BY label ORDER BY label"
    return [dict(row) for row in execute_query(query)]
